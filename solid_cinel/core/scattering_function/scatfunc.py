...
class ScatFuncSD:
    ...
    def from_pdos(cls, Ein: float, M: float, T: float, Eout: np.array,
        ...
        if nphonon:
            ...
            alphaVec = get_alphaFromEout(Eout, Ein, M, T, mu)
            nphonon = get_expansionOrder(alphaVec, DebyeWallerCoeff, decimal, order_max)
        ...
        scatfunc = get_ScatFuncClm(Ein, M, T, Eout, mu, tauN, tauNbeta, DebyeWallerCoeff)[0]
    ...
    def get_alpha0(EinGrid: np.ndarray, M: float, T: float, *args,
        ...
        Ein    alpha    recoil
        6.7554 1.096220 0.028339   0.108530   0.257338   0.204528   0.054488   0.004558
        6.9050 1.120636 0.028971   0.111882   0.255988   0.201056   0.054207   0.004704
        7.0439 1.143306 0.029557   0.114902   0.254680   0.197928   0.053935   0.004837
        7.2000 1.168783 0.030215   0.118190   0.253153   0.194516   0.053616   0.004981
        7.3157 1.187667 0.030704   0.120553   0.251987   0.192055   0.053373   0.005085
        7.4480 1.209259 0.031262   0.123179   0.250621   0.189308   0.053087   0.005201

        >>> ScatFuncSD.get_alpha0(Ein, M, T, pdos, model="sct").iloc[::, 1000::1000].round(6)
        beta                      -3.092990  -1.544947   0.003096   1.551139   3.133205
        Ein    alpha    recoil
        6.7554 1.096220 0.028339   0.110115   0.253051   0.202780   0.053582   0.004631
        6.9050 1.120636 0.028971   0.113346   0.251671   0.199382   0.053295   0.004772
        7.0439 1.143306 0.029557   0.116251   0.250341   0.196320   0.053018   0.004900
        7.2000 1.168783 0.030215   0.119407   0.248795   0.192981   0.052695   0.005038
        7.3157 1.187667 0.030704   0.121672   0.247619   0.190571   0.052449   0.005138
        7.4480 1.209259 0.031262   0.124185   0.246244   0.187882   0.052162   0.005249

        >>> ScatFuncSD.get_alpha0(Ein, M, T, pdos, model="pdos").iloc[::, 1000::1000].round(6)
        beta                      -3.092990  -1.544947   0.003096   1.551139   3.133205
        Ein    alpha    recoil
        6.7554 1.096220 0.028339   0.102532   0.251873   0.213761   0.053320   0.004314
        6.9050 1.120636 0.028971   0.105622   0.250961   0.209850   0.053131   0.004448
        7.0439 1.143306 0.029557   0.108424   0.250037   0.206350   0.052940   0.004570
        7.2000 1.168783 0.030215   0.111492   0.248916   0.202558   0.052707   0.004704
        7.3157 1.187667 0.030704   0.113710   0.248034   0.199838   0.052523   0.004801
        7.4480 1.209259 0.031262   0.116187   0.246977   0.196817   0.052303   0.004910
        """
        ...
        scatfunc.index = pd.MultiIndex.from_arrays([Ein, alpha.data, alpha.get_recoil(T)],
                                                   names=["Ein", "alpha", "recoil"])
        return scatfunc
...
class ScatFuncDD:
    ...
    def to_sd(self, theta: float = None) -> ScatFuncSD:
        ...
        Name: 0.5000000000000001, dtype: float64
        """
        filtAngle = np.cos(np.deg2rad(theta)) if theta else self.get_angle
        scatfunc = self.data.loc[filtAngle]
        scatfunc /= integrate(self.data.loc[filtAngle])
        return ScatFunc(self.Ein, self.T, self.M, scatfunc)
    ...
    def get_angle(self) -> float:
        ...
        if (scatfunc.iloc[::, [0, -1]] >= 1.0e-6).any().any():
            ...
            angularMax = scatfunc.max(axis=1) / scatfunc.apply(integrate, axis=1)
            MD = sigma1(np.array([self.Ein]), self.Ein, self.T, self.M)[0]
            muFitMax = abs(angularMax - MD).idxmin()
            return muFitMax
        else:
...
def scatFuncValuesAlphaVec(Sab_mat: np.ndarray, beta: np.ndarray, Ein: float,
    ...
    -------
    'np.ndarray', (N, 2)
        Scattering function values for a single angle for tauN expansion.
...
def get_ScatFuncClmRow(Ein: float, M: float, T: float, Eout: np.ndarray,
    ...
    >>> tau1beta = pdos.beta_grid(T).data.index.values
    >>> tauNbeta = get_tauNbeta(tau1beta, tauN.shape[1])
    >>> sd_pdf = get_ScatFuncClmRow(Ein, M, T, Eout, mu, tauN, tauNbeta, DebyeWallerCoeff)
