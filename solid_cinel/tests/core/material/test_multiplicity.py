...
@pytest.mark.parametrize("E", [2.301])
def test_multiplicity(E):
    ...
    try:
        ...
        data_original_frame = pd.DataFrame([[h, k, l, d_hkl, Fsq_hkl, mul] for (h, k, l), [d_hkl, Fsq_hkl, mul] in data_original.items()],
                                   columns = ["h", "k", "l", "d", "Fsq", "Multiplicity"]).set_index(["h", "k", "l"]).sort_values(by= ["h", "k", "l"]).round(6)
        assert (Al.get_multiplicity(T, E).round(6) == data_original_frame).all().all()
